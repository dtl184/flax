...
A lot of this was copied and modified from code originally written by Kelsey Allen.
"""
...
def marshalling_func(graph):
    """
    Marshalling function rearranges objects and relations into interaction terms.
...
def aggregation_func(graph):
    """
    Collects all effects that apply to each receiver object, merges them, and combines them
    with the object's attributes and externel effects to produce an input for the object model.
...
class EncodeProcessDecode(GraphModel):
    """This code currently assumes that we only care about the final node values (and not the edge values)
    """
    def __init__(self, dims, num_steps, use_gpu, dropout=0., **kwargs):
        super(EncodeProcessDecode, self).__init__(dims, **kwargs)
    ...
    def forward(self, graph):
        ...
        for steps in range(self.num_steps):
            ...
            output_graph.append(replace(graph, replacements)) #, 'edges':self.edge_decoder(graph['edges'])}))
        return output_graph
...
def setup_graph_net(graph_dataset, use_gpu=False, num_steps=1, layer_size=16, output_dim=1,
                    dropout=0., output_global_dim=None):
    """Create an EncodeProcessDecode graphnet using the dimensions found in the dataset
    """
    ...
    encprocdec = EncodeProcessDecode(dims, num_steps, use_gpu, **layer_dict, dropout=dropout)
